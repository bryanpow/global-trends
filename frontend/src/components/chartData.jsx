...
export const formatDataForPieChart = (arr, data, series) => {
  ...
  for (const region in data) {
    ...
    for (let i = 0; i < years.length; i++) {
      ...
      if (lastYearData[region] !== undefined) {
        ...
        const percentIncrease =
          lastValue !== 0 ? ((currentValue - lastValue) / lastValue) * 100 : 0;

        if (i < midpoint) {
          firstHalfData[region] =
            (firstHalfData[region] || 0) + percentIncrease;
        } else {
          secondHalfData[region] =
            (secondHalfData[region] || 0) + percentIncrease;
        }
      }
    ...
    if (years.length - midpoint > 0) {
      secondHalfData[region] =
        (secondHalfData[region] || 0) / (years.length - midpoint);
    }
  }
